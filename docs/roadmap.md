...
- **✅ FIXED 2026-08-20 — Safari <16.4 got ZERO JavaScript site-wide.** Kept as a case study; the
  portable lessons are in **`docs/web-project-checklist.md`**.
  - **Symptom:** on an iPad Pro (iPadOS 16.2) nothing interactive worked anywhere — hamburger,
    `/dong-hanh` choices, `/giai-dap` filter chips all dead — while cards and links worked normally.
    Identical in Safari and Chrome (same WebKit on iPad), both orientations.
  - **Cause:** the build shipped a **class static block** (`static { … }`, ES2022, Safari 16.4+) inside
    Next's own App Router chunk. On 16.2 that's a parse-time SyntaxError → the chunk never loads →
    React never hydrates → every JS control on every page is inert, while server HTML still renders.
  - **Fix:** `dc8f1e9` — `"browserslist": ["defaults", "safari >= 15"]` in `package.json`.
    **Verified on the deployed site:** 0 `static {` blocks across all chunks on `/giai-dap`.
  - **Key heuristic for next time:** *links work but buttons don't → JS isn't running; stop looking at
    CSS.* Establish that before theorizing — we lost hours to CSS/sticky/layout hypotheses that were
    the wrong category entirely.
  - **Still open, separately:** `a57d078` routes every touch device to the compact header at any width,
    which hides the VI/EN toggle behind the menu. Now that the menu works again, revisit whether that
