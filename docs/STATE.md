...
| 8 | Site Design & Shell | `app/page.tsx` (homepage), `components/SiteHeader`, cross-section **hub** routes that GROUP existing sections (e.g. `app/lich-su-hoi-thanh` = Fathers+Councils) + all entry-point wiring | homepage, global nav, cross-section hubs, IA/design | Church History hub LIVE · Đồng hành entry points **wired** (homepage band + nav item, gated) · **next: add the Saints homepage card + nav link once Session 9 ships Tier 1** |
| 9 | Các Thánh (Saints) | `app/cac-thanh`, `lib/saints*` (mirror `churchFathersV2`), `content/cac-thanh`, `public/images/cac-thanh` | builds the whole Saints section — data model + list/detail pages + content | **Tier 1 LIVE** — data model (`lib/saintsV2` + `lib/saints/groups`, theme-grouped), list + `[slug]` detail + `/cac-thanh/tu-dao-viet-nam` overview, and the 7 VN Martyrs (all HĐGM-verified). Portraits use the `available:false` fallback (PD images = follow-up). **→ Session 8 can now wire the homepage card + nav link.** Next: Tier 2's ~14 (bridge saints' `/phep-la` forward-links already stubbed via `related`) |
