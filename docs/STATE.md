...
| 8 | Site Design & Shell | `app/page.tsx` (homepage), `components/SiteHeader`, cross-section **hub** routes that GROUP existing sections (e.g. `app/lich-su-hoi-thanh` = Fathers+Councils) + all entry-point wiring | homepage, global nav, cross-section hubs, IA/design | Church History hub LIVE · **next: wire the Đồng hành entry points** (import `DongHanhCta` into homepage + nav — the companion is launched at `/dong-hanh` but has NO link yet); then Saints/Miracles cards once those sections exist |
| 9 | Các Thánh (Saints) | `app/cac-thanh`, `lib/saints*` (may mirror `churchFathersV2`), `content/cac-thanh`, `public/images/cac-thanh` | builds the whole Saints section — data model + list/detail pages + content | **ready to start** (lineup spec'd in `docs/roadmap.md` "Session 9 — Các Thánh: starting lineup") — not yet spawned |
